...
@section('head_title'){{__('Add Question')}}@endsection
@section('content')
...
<div class="card">
    <div class="card-header">
        <h5 class="card-title"> @lang("Add Question") </h5>
    ...
    <div class="card-body table-responsive">
        <form class="form-horizontal" action="{{route('app.common_questions.store')}}" method="post" enctype="multipart/form-data">
            <fieldset>
                @csrf
                @foreach(app_languages() as $key=>$one)
                    <div class="tab-pane {{ $key == app()->getLocale() ? 'active' : '' }}" id="tab-{{$key}}" role="tabpanel">
                        <div class="form-body">
                            <div class="form-group row">
                                {!! Inputs('text', $key.'[question]', 'Question', 'form-control', old($key.'.question')) !!}
                            </div>
                            <div class="form-group row">
                                {!! TextArea($key.'[answer]', 'Answer', 'form-control', old($key.'.answer'), true, $key.'_answer') !!}
                            </div>
                        </div>
                    </div>
                @endforeach
...
@push('scripts')
    ...
    {!! JsValidator::formRequest('App\Http\Requests\Dashboard\Questions\StoreRequest') !!}
