...
@section('head_title'){{__('Edit City')}}@endsection
@section('content')
...
<div class="card">
    <div class="card-header">
        <h5 class="card-title"> @lang("Edit City") </h5>
    ...
    <div class="card-body table-responsive">
        <form class="form-horizontal" action="{{route('app.cities.update', $city->id)}}" method="post" enctype="multipart/form-data">
            <fieldset>
                @csrf
                @method('PUT')
                @foreach(app_languages() as $key=>$one)
                    <div class="tab-pane {{ $key == app()->getLocale() ? 'active' : '' }}" id="tab-{{$key}}" role="tabpanel">
                        <div class="form-body">
                            <div class="form-group row">
                                {!! Inputs('text', $key.'[name]', 'Name', 'form-control', old($key.'.name', $city->translate($key)->name??'')) !!}
                            </div>
                        </div>
                    </div>
                @endforeach
