...
@section('head_title'){{__('Payment Methods Images')}}@endsection
@section('content')
@include('admin.layouts.inc.breadcrumb', ['array' => [
    [
        ...
        'title' => __('Add Payment Methods Image'),
                        ...
                        <table class="table datatable-basic">
                            ...
                            <tbody>
                                ...
                                @foreach ($lists as $item)
                                    <tr>
                                        <td>{{$item->id}}</td>
                                        <td> <img src="{{$item->image_path}}" alt="{{$item->image_path}}" style="height: 70px !important;"> </td>
                                        <td>
                                            @if($item->is_active == 0)
                                                <a href="{{route('app.payment_methods_images.is_active', $item->id)}}" class="label label-sm label-danger" title="{{__('Active')}}"> <i data-feather="x" stroke-width="7" style="color: red;"></i> </a>
                                            @else
                                                <a href="{{route('app.payment_methods_images.is_active', $item->id)}}" class="label label-sm label-success" title="{{__('Un Active')}}"> <i data-feather="check" stroke-width="7" style="color: #00b800;"></i> </a>
                                            @endif
                                        </td>
                                        <td> {{$item->created_at}} </td>
                                        <td>
                                            {!! editForm('payment_methods_images', $item) !!}
                                            {!! deleteForm('payment_methods_images', $item) !!}
                                        </td>
                                    </tr>
                                @endforeach
                            @else
