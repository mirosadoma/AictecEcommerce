...
@section('head_title'){{__('Categories')}}@endsection
@section('content')
                        ...
                        <table class="table datatable-basic">
                            <thead>
                                <tr>
                                    ...
                                    <th {!! \table_width_head(7) !!}>@lang('View In Site')</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($lists as $item)
                                    <tr>
                                        ...
                                        <td> {{$item->name ?? '-------'}} </td>
                                        <td> {{$item->in_site == 1 ? __("Yes") : __('No')}} </td>
                                        <td>
                                            @if($item->is_active == 0)
                                                <a href="{{route('app.categories.is_active', $item->id)}}" class="label label-sm label-danger" title="{{__('Active')}}"> <i data-feather="x" stroke-width="7" style="color: red;"></i> </a>
                                            @else
                                                <a href="{{route('app.categories.is_active', $item->id)}}" class="label label-sm label-success" title="{{__('Un Active')}}"> <i data-feather="check" stroke-width="7" style="color: #00b800;"></i> </a>
                                            @endif
                                        </td>
                                        <td> {{$item->created_at}} </td>
                                        <td>
                                            {!! editForm('categories', $item) !!}
                                            {!! deleteForm('categories', $item) !!}
                                        </td>
                                    </tr>
                                @endforeach
                            @else
