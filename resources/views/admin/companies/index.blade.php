...
@section('head_title'){{__('All Companies')}}@endsection
@section('content')
                        ...
                        <form class="dt_adv_search" method="GET">
                            <div class="row g-1 mb-md-1">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" name="company_name" class="form-control dt-input dt-full-name" value="{{old('company_name', request('company_name'))}}" data-column="1" placeholder="{{__('Company Name')}}" data-column-index="0" />
                                </div>
                                <div class="col-md-4">
                        ...
                        <table class="table datatable-basic">
                            <thead>
                                <tr>
                                    ...
                                    <th>@lang('Name')</th>
                                    <th {!! \table_width_head(6) !!}>@lang('Company Name')</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($lists as $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if($item->is_active == 0)
                                                <a href="{{route('app.companies.is_active', $item->id)}}" class="label label-sm label-danger" title="{{__('Active Company')}}"> <i data-feather="x" stroke-width="7" style="color: red;"></i> </a>
                                            @else
                                                <a href="{{route('app.companies.is_active', $item->id)}}" class="label label-sm label-success" title="{{__('Un Active Company')}}"> <i data-feather="check" stroke-width="7" style="color: #00b800;"></i> </a>
                                            @endif
                                        </td>
                                        <td> {{$item->created_at}} </td>
                                        <td>
                                            {!! editForm('companies', $item) !!}
                                            @if(!request()->has('type') || request('type') != "deleted")
                                                {!! deleteForm('companies', $item) !!}
                                            @elseif(request()->has('type') && request('type') == "deleted")
                                                {!! restoreForm('companies', $item) !!}
                                                {!! destroyForm('companies', $item) !!}
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
