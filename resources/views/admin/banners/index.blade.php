...
@section('head_title'){{__('Banners')}}@endsection
@section('content')
                        ...
                        <form class="dt_adv_search" method="GET">
                            <div class="row g-1 mb-md-1">
                                <div class="col-md-4">
                                    ...
                                    <input type="text" name="link" class="form-control dt-input dt-full-name" value="{{old('link', request('link'))}}" data-column="1" placeholder="{{__('Link')}}" data-column-index="0" />
                                </div>
                                <div class="col-md-4">
                        ...
                        <table class="table datatable-basic">
                            ...
                            <tbody>
                                ...
                                @foreach ($lists as $item)
                                    <tr>
                                        <td>{{$item->id}}</td>
                                        <td> <img src="{{$item->image_path}}" alt="{{$item->image_path}}" style="height: 100px !important;"> </td>
                                        <td> {{$item->link ?? '-------'}} </td>
                                        <td>
                                            @if($item->is_active == 0)
                                                <a href="{{route('app.banners.is_active', $item->id)}}" class="label label-sm label-danger" title="{{__('Active')}}"> <i data-feather="x" stroke-width="7" style="color: red;"></i> </a>
                                            @else
                                                <a href="{{route('app.banners.is_active', $item->id)}}" class="label label-sm label-success" title="{{__('Un Active')}}"> <i data-feather="check" stroke-width="7" style="color: #00b800;"></i> </a>
                                            @endif
                                        </td>
