                        ...
                        <table class="table datatable-basic">
                            <thead>
                                <tr>
                                    ...
                                    <th>@lang('Name')</th>
                                    <th {!! \table_width_head(7) !!}>@lang('Price')</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($lists as $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if($item->is_active == 0)
                                                <a href="{{route('app.shipping_companies.is_active', $item->id)}}" class="label label-sm label-danger" title="{{__('Active')}}"> <i data-feather="x" stroke-width="7" style="color: red;"></i> </a>
                                            @else
                                                <a href="{{route('app.shipping_companies.is_active', $item->id)}}" class="label label-sm label-success" title="{{__('Un Active')}}"> <i data-feather="check" stroke-width="7" style="color: #00b800;"></i> </a>
                                            @endif
                                        </td>
                                        <td> {{$item->created_at}} </td>
                                        <td>
                                            {!! editForm('shipping_companies', $item) !!}
                                            {!! deleteForm('shipping_companies', $item) !!}
                                        </td>
                                    </tr>
                                @endforeach
                            @else
