...
@section('head_title'){{__('Brands')}}@endsection
@section('content')
                            ...
                            <tbody>
                                ...
                                @foreach ($lists as $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if($item->is_active == 0)
                                                <a href="{{route('app.brands.is_active', $item->id)}}" class="label label-sm label-danger" title="{{__('Active')}}"> <i data-feather="x" stroke-width="7" style="color: red;"></i> </a>
                                            @else
                                                <a href="{{route('app.brands.is_active', $item->id)}}" class="label label-sm label-success" title="{{__('Un Active')}}"> <i data-feather="check" stroke-width="7" style="color: #00b800;"></i> </a>
                                            @endif
                                        </td>
                                        <td> {{$item->created_at}} </td>
                                        <td>
                                            {!! editForm('brands', $item) !!}
                                            {!! deleteForm('brands', $item) !!}
                                        </td>
                                    </tr>
                                @endforeach
                            @else
